...
or nil if none found within MAX-D."
  (let* ((beg (point-min))
    ...
    (catch 'ret
      (dotimes (i max-d)
        (let ((current-pos (+ pos (* i dir))))
          (when (if backward (>= current-pos beg) (< current-pos end))
            (when-let* ((syn (syntax-after current-pos))
                        (class (syntax-class syn))
                        ((or (= class target-class)
                             (and (= class 7)
                                  pair-hl-highlight-string-quotes))))
              (condition-case nil
                  (let* ((start-pos (if backward (1+ current-pos) current-pos))
